...
class MyThreadWithArgs(threading.Thread):
    ...
    def run(self):
        ...
        if (len(self.args) > 1):
            ...
            wait_for_event_timeout_to_readFile(self.args[0], self.args[1])

            if self.mWindow != None:
                for i in range(10000):
                    self.mWindow.FindElement('-PROGBAR-').UpdateBar(i + 1)
...
def print_time(a='default'):
    print("From print_time", datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"), a)


def timer_one_event(name):
    global timerOne
    print('EVENT:===========5t43t43tt3t4334t4', print_time('EVENT'), name)
    timerOne = False


def mysleeptimerTarget(waitTime):
    time.sleep(int(waitTime))


def mysleeptimer(waitTime):
    t1 = threading.Thread(target=mysleeptimerTarget, args=(waitTime,))
    t1.start()
    t1.join()


def runSleepTimer(t1, t2):
    global timerOne


def wait_for_event_timeout_to_readFile(e, config):
    """Wait t seconds and then timeout"""
    global timerOne
    t = 1
    t = threading.Thread(target=scheduler.run)
    # while True:
    runSleepTimer(1, int(config.waitTime))
    logging.debug('wait_for_event_timeout is starting wait_for_event_timeout_to_readFile_readFile Wait')
    # event_is_set = e.wait(t)

    if (timerOne):

        print('START:', print_time())
        e1 = scheduler.enter(1, 2, timer_one_event, ('E1',))
        e2 = scheduler.enter(int(config.waitTime), 1, timer_one_event, ('E2',))

        # Start a thread to run the events
        # t.start()
        mysleeptimer(config.waitTime)
    else:

        logging.debug(
            "Wait %s seconds and then timeout :Time now is  %s ",
            config.waitTime,
            datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        logging.debug("wait_for_event_tim"
                      "eout event wait state %d seconds and then timeout :Time now is  %s ", e.is_set(),
                      datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        timerOne = True
...
# def getImagesAndDisplay(dir, mWin):
def getImagesAndDisplay(configData, mWin):
    url = ''
    # imageInfo = downLoadImageHelper.downLoadImage(url,dir)
    for i in range(10000):
        mWin.FindElement('-PROGBAR-').UpdateBar(i + 1)
...
def get_img_data(f, maxsize=(1400, 850), first=False):
...
def updateMyConfigyData(mWindow, myConfig):
    ...
    print(
        "myConfig Data is [{}{}{}{}]".format(myConfig.server, myConfig.userName, myConfig.password, myConfig.waitTime))
    # myConfig.stopTime = mWindow["-SERVER-"].get()
    # myConfig.storeDir = mWindow["-SERVER-"].get()


def main():
    global isMyThreadsRunning
    ...
    column1 = [[sg.Text("TekVizion Gabber Version 0.1", font=("Helvetica", 15), text_color="#FF0000", justification="center")],]
    ProgBar = [[sg.Text('progress meter')],
                [sg.ProgressBar(10000, orientation='h', size=(10, 10), key='-PROGBAR-')],
                [sg.Cancel(button_color=MOCOLOR)]]
    ...
    elements = [
        ...
        [sg.Text('Grabber v.0.1', background_color=MOCOLOR, text_color='white', k='-TITLEBAR-', size=(115, 1),
        ...
        [sg.Button('Submit', button_color=MOCOLOR), sg.Button('Cancel', button_color=MOCOLOR),
         sg.Exit(button_color=MOCOLOR)],
        [sg.Text('_' * 80)],
        [sg.Image(key="image")],
        [
    ...
    layOut = ProgBar+ elements + layout1
    #mWindow = sg.Window("Grabber Viewer", layOut, size=(675, 975), resizable=1, keep_on_top=True)
    mWindow = sg.Window("Grabber Viewer", layOut, size=(675, 975), resizable=1, keep_on_top=False)
    # create the form
    ...
    mDirPath = ''
    i = 0
    missingConfigData = True  # when the user hit submit and if the values entered are valid we flip this to False
    ...
    while True:
        ...
        if (isMyThreadsRunning):
                getLatestDownloadedFile(myConfig.storeDir, mWindow)

        event, values = mWindow.read(timeout=10000)
        print(event, values)
        if event == "-PROGBAR-":
            #TBD ALex use this to hide the PG bar.
            mWindow["-PROGBAR-"].update(data=bio.getvalue())
            break
        ...
        if event == "Submit":
            ...
            if not isMyThreadsRunning:
                ...
                if (myConfig.server == '') or (myConfig.waitTime == '') or (myConfig.userName == '') or (
                        myConfig.password == ''):
                    sg.SetOptions(#window_location=(700, 400),
                                  margins=(10, 10),
                                  button_color=('Red', 'gainsboro'))
                    sg.Popup('Error Missing Config Data, will use saved config for now')  # Shows red error button

                    print("Submit Event missing some critical config data")
                    # for testing
                    ...
                    myConfig.storeDir = '/Users/AlexAltaraireh/Desktop'  # /Users/mohammadaltaraireh/Desktop/images/'
                    mWindow["-DIR-"].update(myConfig.storeDir)
                    mfile = myConfig.storeDir

                    missingConfigData = True
                else:
                    missingConfigData = False
                    #sg.Popup('NO Missing Config Data Running ', keep_on_top=True)  # Shows red error button

            if (not isMyThreadsRunning) and missingConfigData == False:
                print('Hit Submit isMyThreadsRunning should be False [ %s ] and missingConfigData should be False  [%s]', isMyThreadsRunning, missingConfigData)
                #sg.Popup('Config Data, will use saved config for now', keep_on_top=True)  # Shows red error button
                isMyThreadsRunning = True

                mDirPath = mWindow["-DIR-"].get()
                t1 = threading.Thread(target=getImagesAndDisplay, args=(myConfig, mWindow))
                t2 = threading.Thread(target=refreshImageFiles, args=(myConfig, mWindow))
                t1.start()
                t2.start()

                twait1 = MyThreadWithArgs(name='twait1', target=wait_for_event_timeout_to_readFile,
                                          args=(tEvent, myConfig, mWindow), kwargs={'c': 'C', 'd': 'D'})
                twait1.start()
