...
#"display_image_in_window" method called after the timer thread sends an event
...
#"get_image_via_url_and_display" method called after a timer thread sends an event to get an image from a configured URL or server address
def get_image_via_url_and_display(myConfig, window):
    if (myConfig.server != ''):
        url = "http://10.1.53.83/screencapture" #default url at the moment

    #time1 = the time before sending the get request to the server
    time1 = datetime.datetime.now() #get current time before request
    get_time = ('getImageViaUrlAndDisplay: time of get request = ' + time1.strftime("%Y-%m-%d-%H-%M-%S.%f")[:-3])
    response = None

    #the server address, user name and password from the GUI
    mheaders={
    ...
    #----Information here will be displayed in a log for accuracy----
    #time2 = the time after we received a response from the server
    time2 = datetime.datetime.now()
    current_time = ('getImageViaUrlAndDisplay: time of get response = ' + time2.strftime(
        "%Y-%m-%d-%H-%M-%S.%f")[:-3])

    #time to get the image
    timeToGetImage = time2 - time1

    sg.cprint(current_time, text_color='white', background_color='blue') #displaying time of request
    sg.cprint(timeToGetImage, text_color='white', background_color='blue') #displaying time of response
...
def grabber_main():
    ...
    column1 = [[sg.Text("TekVizion Grabber Version 0.1", font=("Helvetica", 15), text_color="#FF0000",
                        justification="center")], ]

    elements = [
        # [sg.Check('Debug Mode', k='-DEBUGMODE-', size=(115, 1))],
        [sg.Image(data=LOGO)],
        [sg.Column(column1, size=size)],
        [sg.Text('Grabber v.0.1', background_color=NEWCOLOR, text_color='white', k='-TITLEBAR-', size=(115, 1),
                 justification='center')],
        [sg.Text('Server Address', size=(20, 1)), sg.InputText(key='-SERVER-', size=(40, 1), do_not_clear=True)],
        [sg.Text('User Name', size=(20, 1)), sg.InputText(key='-NAME-', size=(40, 1), do_not_clear=True)],
        [sg.Text('Password', size=(20, 1)), sg.InputText(key='-PASSWORD-', password_char='*', size=(40, 1), do_not_clear=True)],
        [sg.Text('Frequency in Seconds', size=(20, 1)), sg.InputText(key='-REFERESH-', size=(4, 1), do_not_clear=True)],
        [sg.Button('Submit', k='-SUBMIT-', button_color=NEWCOLOR), sg.Exit(button_color=NEWCOLOR)],
        [sg.Text('_' * 80)],
    ]
    ...
    window = sg.Window('TekVizion Image Grabber', finalLayout, size=(675, 975), resizable=1, keep_on_top=False,
                       finalize=True)
    ...
    while True:
        ...
        if event == "-SUBMIT-":
            ...
            if (haveAllConfig):
                ...
                sg.Popup('Missing Config Data')  # Shows red error button
